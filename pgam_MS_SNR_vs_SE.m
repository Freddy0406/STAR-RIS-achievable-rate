function [finalSE] = pgam_MS_SNR_vs_SE(SNR,RIS_element,wavelen_coef)
    %% Configuration
    ...
    %% Quantization for beta
    for n = 1:N
        if(abs(prev.beta_t(n))>abs(prev.beta_r(n)))
            prev.beta_t(n) = 1;
            prev.beta_r(n) = 0;
        else
            prev.beta_t(n) = 0;
    ...
    new_Phi_t = diag(prev.theta_t.*prev.beta_t);
    new_Phi_r = diag(prev.theta_r.*prev.beta_r);
    ...
    [~,~,~,finalSE] = f_function(new_Rk,new_Psik,K,noise_var,rho_linear);

    finalSE = sum_SE_coef * finalSE;
