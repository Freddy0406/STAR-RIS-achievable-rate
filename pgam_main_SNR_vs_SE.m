function [final_SE] = pgam_main_SNR_vs_SE(SNR)
    ...
    [loc_RIS,N,~,element_size] = configRIS(wave_length);
    ...
    while true  % Outer loop
        ...
        for i = 1:K
            nabla_theta_t_f = nabla_theta_t_f+nabla_sum_SE_coef*(interference(i)*nabla_theta_t_sk(:,:,i)-signal(i)*nabla_theta_t_Ik(:,:,i))/((1+gamma_k(i))*interference(i)^2);
            nabla_theta_r_f = nabla_theta_r_f+nabla_sum_SE_coef*(interference(i)*nabla_theta_r_sk(:,:,i)-signal(i)*nabla_theta_r_Ik(:,:,i))/((1+gamma_k(i))*interference(i)^2);
        end
        ...
        for i = 1:K
            nabla_beta_t_f = nabla_beta_t_f+nabla_sum_SE_coef*(interference(i)*nabla_beta_t_sk(:,:,i)-signal(i)*nabla_beta_t_Ik(:,:,i))/((1+gamma_k(i))*interference(i)^2);
            nabla_beta_r_f = nabla_beta_r_f+nabla_sum_SE_coef*(interference(i)*nabla_beta_r_sk(:,:,i)-signal(i)*nabla_beta_r_Ik(:,:,i))/((1+gamma_k(i))*interference(i)^2);
        end
        ...
        while true  
            ...
            [new_f_func] = f_function(new_Rk,new_Psik,K,SNR_linear);
